    ...
    <div class="vtb-item-players">
        <div>
            <div :class="['vtb-player', 'vtb-player1', getPlayerClass(bracketNode.player1)]"
                @mouseover="highlightPlayer(bracketNode.player1.id)" @mouseleave="unhighlightPlayer">
                <!-- Display player 1's name and scores -->
                <slot :player="bracketNode.player1" name="player">
                    {{ bracketNode.player1.name }} ({{ bracketNode.player1.scores.join(", ") }})
                </slot>
            </div>

            <div :class="['vtb-player', 'vtb-player2', getPlayerClass(bracketNode.player2)]"
                @mouseover="highlightPlayer(bracketNode.player2.id)" @mouseleave="unhighlightPlayer">
                <!-- Display player 2's name and scores -->
                <slot :player="bracketNode.player2" name="player">
                    {{ bracketNode.player2.name }} ({{ bracketNode.player2.scores.join(", ") }})
                </slot>
            </div>
        </div>
